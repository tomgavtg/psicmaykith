...
export function formatWhatsAppNumber(number) {
  ...
  if (/^52\d{10}$/.test(normalizedNumber)) {
    const nationalNumber = normalizedNumber.slice(2);
    return `+52 ${nationalNumber.slice(0, 2)} ${nationalNumber.slice(2, 6)} ${nationalNumber.slice(6)}`;
  }

  return /^\d{10,15}$/.test(normalizedNumber) ? `+${normalizedNumber}` : "";
}
